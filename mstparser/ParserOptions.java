...
public final class ParserOptions {
    ...
    public String testfile = "data/web.lab";
    public File trainforest = null;
    public File testforest = null;
    public File in = null;
    public File out = null;
    public String fmt = "xml";
    public boolean train = false;
    public boolean eval = false;
    public boolean test = true;
    public String modelName = "lib/dep18.model";
    ...
    public String outfile = "data/out.txt";
    ...
    public boolean perceptron = false;
	...
	for(int i = 0; i < args.length; i++) {
	    ...
	    if (pair[0].equals("in")) {
	    	...
	    	if (!in.exists() || in.isDirectory()) {
	    		System.err.println("Invalid input file: " + pair[1]);
	    		System.exit(1);
	    	}
	    }
	    if (pair[0].equals("out")) {
	    	out = new File(pair[1]);
	    	if (!out.exists() || out.isDirectory()) {
	    		System.err.println("Invalid out file: " + pair[1]);
	    		System.exit(1);
	    	}
	    }
	    if (pair[0].equals("fmt")) {
	    	fmt = pair[1].toLowerCase();
	    	if (!fmt.equals("xml") && !fmt.equals("json")) {
	    		System.err.println("Invalid output format: " + fmt + ", only [xml] and [json] are available");
	    		System.exit(1);
	    	}
	    }
	    if(pair[0].equals("preload")){
	    	preload = Boolean.parseBoolean(pair[1]);
	    }
	    if(pair[0].equals("perceptron")){
	    	perceptron = Boolean.parseBoolean(pair[1]);
	    }
