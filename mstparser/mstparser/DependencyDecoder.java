...
public class DependencyDecoder {
    ...
    DependencyPipe pipe;
//  added by Wang Liang 
    public static String[][][] maxTypes = new String[400][][];
...
//	added by Wang Liang
	if(DependencyParser.evaluting == true){
		DependencyDecoder.maxTypes[current] = new String[len][len];	
	
		for(int i=0; i<maxTypes[current].length; i++)
			for(int j=0; j<maxTypes[current][i].length; j++)			
				DependencyDecoder.maxTypes[current][i][j] = "";
	}
//	ended
	for(int i = 0; i < len; i++) {
	    for(int j = 0; j < len; j++) {
...
//		    modified by Wang Liang
		    if(score > best) { 
		    	wh = t; best = score;
//		    	added by Wang Liang
		    	if(DependencyParser.evaluting)
		    		maxTypes[current][i][j] = pipe.types[t];
		    }
		}
		static_types[i][j] = wh;
	    }
